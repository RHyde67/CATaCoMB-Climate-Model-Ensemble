...
%% Simple MMM Bias
Folder = ('..\Outputs\MonthlyBiasSimple');
...
for Month = 1:12
    ...
    caxis([-10,10])
    title(sprintf('(%s) Bias Simple MMM, Month %i\n Mean Bias %.2f (DU)', char(Month+96), Month, mean(Bias(:)) ))
...
set(gcf, 'Color', 'w');
hFMontage = figure(99);
export_fig(hFMontage, '-m2', 'MonthlyBiasValues');
end